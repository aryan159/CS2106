...
These are provided "myfree" to you, and you do not need to implement them,
...
TTreeNode *makeNewNode(char *name, char *phoneNum)
{
    ...
    TTreeNode *newNode = (TTreeNode *)mymalloc(sizeof(TTreeNode));
    newNode->name = (char *)mymalloc(strlen(name) + 1);
    // newNode->phoneNum = (char *)mymalloc(strlen(phoneNum) + 1);
